...
// TODO 2025-02-24: Добавить описание параметра ПараметрыФормы
...
// РЕЖИМРАЗРАБОТКИ:&После("МодифицироватьФорму")
// РЕЖИМРАЗРАБОТКИ:Процедура МодифицироватьФорму()
// КОНЕЦ БЛОКА РЕЖИМ РАЗРАБОТКИ
Процедура МодифицироватьФорму(Форма, ПараметрыФормы, Отказ, ИмяМакетаОписанияМодификаций, ГенераторПротокола = Неопределено) Экспорт
	Протокол = МФСлужебный.Протокол(ГенераторПротокола);
	//Протокол.Форма = Форма;
	...
	ДополнительнаяОбработкаДанныхДляТестирования(Форма, ПараметрыФормы, ОписаниеМодификаций);
	
	// НАЧАЛО БЛОКА РЕЖИМ РАЗРАБОТКИ
	Если ПараметрыФормы.Свойство(МФИнструментыРазработчикаКлиентСервер.ИмяПараметраГенерацияАвтотестов()) Тогда
		АдресДанныхДляГенерацииАвтотестов = ПараметрыФормы[
...
// НАЧАЛО БЛОКА РЕЖИМ ТЕСТИРОВАНИЯ
// РЕЖИМТЕСТИРОВАНИЯ:&После("ДополнительнаяОбработкаДанныхДляТестирования")
// РЕЖИМТЕСТИРОВАНИЯ:Процедура ДополнительнаяОбработкаДанныхДляТестирования(Форма, ПараметрыФормы, ОписаниеМодификаций)
// КОНЕЦ БЛОКА РЕЖИМ ТЕСТИРОВАНИЯ
Процедура ДополнительнаяОбработкаДанныхДляТестирования(Форма, ПараметрыФормы, ОписаниеМодификаций)
	// НАЧАЛО БЛОКА РЕЖИМ ТЕСТИРОВАНИЯ
	Если Не ПараметрыФормы.Свойство(МФТестированиеКлиентСерверПовтИсп.ИмяПараметраТестирования()) Тогда
		Возврат;
	КонецЕсли;
	
	АдресДанныхДляТестирования = ПараметрыФормы[
