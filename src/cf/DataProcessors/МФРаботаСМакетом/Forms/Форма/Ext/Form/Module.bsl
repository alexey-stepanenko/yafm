...
Процедура ПолноеИмяФормыПриИзменении(Элемент)
	ПолноеИмяФормыПриИзмененииНаСервере();
...
Процедура ОткрытьМодифицируемуюФорму(Команда)
	...
	Если Не ((СсылкаНаОбъект = Неопределено) Или СсылкаНаОбъект.Пустая()) Тогда
		ПараметрыОткрытияФормы.Вставить("Ключ", СсылкаНаОбъект);
	КонецЕсли;
	ОткрытьФорму(ИмяРедактируемойФормы, ПараметрыОткрытияФормы);
...
Процедура УстановитьВидимостьДоступностьЭлементов()
	...
	Элементы.СсылкаНаОбъект.Видимость = (
		Элементы.СсылкаНаОбъект.ОграничениеТипа <> Новый ОписаниеТипов("Неопределено")
	);
КонецПроцедуры
...
&НаСервере
Процедура ПолноеИмяФормыПриИзмененииНаСервере()
	Элементы.СсылкаНаОбъект.ОграничениеТипа = Новый ОписаниеТипов("Неопределено");
	СсылкаНаОбъект = Неопределено;
	// TODO 2026-04-12: Добавить другие ссылочные объекты
	СсылочныеКоллекцииОбъектовМетаданных = Новый Массив;
	СсылочныеКоллекцииОбъектовМетаданных.Добавить(Метаданные.Справочники);
	СсылочныеКоллекцииОбъектовМетаданных.Добавить(Метаданные.Документы);
	ОбъектМетаданныхФорма = Метаданные.НайтиПоПолномуИмени(ИмяРедактируемойФормы);
	Если ОбъектМетаданныхФорма = Неопределено Тогда
		Возврат;
	КонецЕсли;
	Нашли = Ложь;
	Для Каждого КоллекцияОбъектовМетаданных Из СсылочныеКоллекцииОбъектовМетаданных Цикл
		Для Каждого ОбъектМетаданных Из КоллекцияОбъектовМетаданных Цикл
			Если ОбъектМетаданных.Формы.Содержит(ОбъектМетаданныхФорма) Тогда
				Нашли = Истина;
				Прервать;
			КонецЕсли;
		КонецЦикла;
	...
	ИмяТипа = СтрШаблон(
		"%1Ссылка%2",
		Лев(ИмяТипа, ПозицияТочки - 1),
		Прав(ИмяТипа, СтрДлина(ИмяТипа) - ПозицияТочки + 1)
	);
	Элементы.СсылкаНаОбъект.ОграничениеТипа = Новый ОписаниеТипов(ИмяТипа);
КонецПроцедуры
