...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИмяРасширения = МФИнструментыРазработчикаКлиентСервер.ИмяРасширенияТестирования();
...
Процедура ПриОткрытии(Отказ)
	...
	КонецЕсли;
	ПутьКФайлуЗапуска1СПоУмолчанию = СтрШаблон(
		"%1%2%3",
		КаталогПрограммы(),
		"1cv8",
		СуффиксФайлаЗапуска
	...
	Если ПустаяСтрока(ПутьКФайлуЗапуска1С) Тогда
		ПутьКФайлуЗапуска1С = ПутьКФайлуЗапуска1СПоУмолчанию;
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	Если ИмяСобытия = "УспешноеЗавершениеСозданияРасширенияСТестами" Тогда
		Обработка_УспешноеЗавершениеСозданияРасширенияСТестами(Параметр);
	ИначеЕсли ИмяСобытия = "ОшибкаСозданияРасширенияСТестами" Тогда
		Обработка_ОшибкаСозданияРасширенияСТестами(Параметр);
	ИначеЕсли ИмяСобытия = "УспешноеЧтениеДополнительныхСвойствРасширенияСТестами" Тогда
		Обработка_УспешноеЧтениеДополнительныхСвойствРасширенияСТестами(Параметр);
	ИначеЕсли ИмяСобытия = "ОшибкаЧтенияДополнительныхСвойствРасширенияСТестами" Тогда
		Обработка_ОшибкаЧтенияДополнительныхСвойствРасширенияСТестами(Параметр);
	Иначе
		// Пустой блок
	КонецЕсли;
КонецПроцедуры
...
Процедура ТестируемыеФормыПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	ЗаполнитьСвойстваТестируемойФормы(Элементы.ТестируемыеФормы.ТекущиеДанные);
...
Процедура ПараметрыГенерацииРасширения(Команда)
	...
	Для Каждого ИмяПараметра Из ИменаПараметров Цикл
		ПараметрыОткрытияФормы.Вставить(ИмяПараметра, ЭтотОбъект[ИмяПараметра]);
	КонецЦикла;
	ОповещениеОЗакрытии = Новый ОписаниеОповещения("ПослеЗакрытияФормыПараметровГенерацииРасширения", ЭтотОбъект);
	ОткрытьФорму(
...
Процедура СоздатьРасширениеСТестами(Команда)
	...
	// TODO 2026-01-04: Сбросить флаг при обновлении расширения, дополнительно проверить версию расширения
...
// УДАЛИТЬ ИЗ РЕЛИЗА: КОНЕЦ БЛОКА
	...
	Для Каждого ИмяСвойства Из СтрРазделить(ИменаСвойствГенерацииРасширения, ",") Цикл
		СвойстваГенерацииРасширения.Вставить(ИмяСвойства, ЭтотОбъект[ИмяСвойства]);
	КонецЦикла;
	СвойстваГенерацииРасширения.Вставить("ДоступностьФормироватьМодулиСТестами", ДоступностьФормироватьМодулиСТестами);
	ПараметрыОткрытияФормы = Новый Структура(
		"СвойстваГенерацииРасширения",
		СвойстваГенерацииРасширения
	);
	ОписаниеОповещенияОЗакрытии = Новый ОписаниеОповещения("ПослеУстановкиСвойствГенерацииРасширения", ЭтотОбъект);
	ОткрытьФорму(
...
Процедура ПоказатьФорму(Команда)
	Если Не ПустаяСтрока(ИмяТестируемойФормы) Тогда
		ОткрытьФорму(ИмяТестируемойФормы, , , УникальныйИдентификатор);
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Асинх Процедура ЗагрузитьИзКэша(Команда)
	ФормыИзКэша = ФормыИзКэша();
	Если ФормыИзКэша.Количество() = 0 Тогда
		РезультатВопроса = Ждать ВопросАсинх(
			НСтр("ru='Необходимо заполнить кэш. Открыть форму?'"),
		...
		Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
			ОткрытьФорму("РегистрСведений.МФМакеты.Форма.ФормаСписка");
			
			Возврат;
		КонецЕсли;
	КонецЕсли;
	РезультатВопроса = Ждать ВопросОчисткиСпискаТестов();
	Если РезультатВопроса <> КодВозвратаДиалога.Да Тогда
		Возврат;
	КонецЕсли;
	ТестируемыеФормы.Очистить();
	ФормыИзКэша = ФормыИзКэша();
	Для Каждого ИмяФормыИзКэша Из ФормыИзКэша Цикл
		НоваяСтрока = ТестируемыеФормы.Добавить();
		НоваяСтрока.ИмяФормы = ИмяФормыИзКэша;
		НоваяСтрока.ФормаОтсутствует = ФормаОтсутствует(ИмяФормыИзКэша);
		
		ЗаполнитьСвойстваТестируемойФормы(НоваяСтрока);
	КонецЦикла;
КонецПроцедуры
...
Асинх Процедура ЗагрузитьИзРасширения(Команда)
	...
	ПолныйПутьКФайлуРасширения = ИмяФайлаРасширения;
	...
	Для Каждого ПараметрИнициализации Из ПараметрыИнициализации Цикл
		Если ИменаПараметров.Найти(ПараметрИнициализации.Ключ) = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		ПараметрыИнициализации[ПараметрИнициализации.Ключ] = ЭтотОбъект[ПараметрИнициализации.Ключ];
	КонецЦикла;
	ПараметрыИнициализации.ИмяРасширения = СвойстваРасширения.Имя;
	ГенераторРасширения.Инициализировать(ПараметрыИнициализации);
	
	ГенераторРасширения.Открыть();
	ПодключитьОбработчикОжидания("ЗагрузитьИзРасширения_Выполнение", 0.1, Истина);
КонецПроцедуры
...
Процедура ПослеЗакрытияФормыПараметровГенерацииРасширения(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	Для Каждого ИмяПараметра Из ИменаПараметров Цикл
		...
		СохраняемыеНастройки.Вставить(ИмяСохраняемогоПараметра, ЭтотОбъект[ИмяПараметра]);
	КонецЦикла;
...
Процедура УстановитьВидимостьДоступностьЭлементов()
	...
	Элементы.ПоказатьФорму.Доступность = Не ПустаяСтрока(ИмяТестируемойФормы);
...
Процедура ОбновитьОформлениеЭлементов()
	...
	Иначе
		...
		Если ФормаОтсутствует Тогда
			Элементы.СтатусФормы.Заголовок = НСтр("ru='Форма отсутствует в конфигурации'");
		Иначе 
			Элементы.СтатусФормы.Заголовок = НСтр("ru='Форма не использует механизм модификации'");
		КонецЕсли;
	КонецЕсли;
...
Функция ПараметрыФормыИзКэша(_ИмяФормы, Шаблоны, АдресДанныхДляГенерацииАвтотестов, ЯзыкГенерации)
	...
	Исключение
		// TODO 2026-01-08: Сделать обработку ошибки
		ВызватьИсключение;
	КонецПопытки;
...
Процедура УстановитьПараметрыТекущейФормы()
	...
	Иначе
		...
		ФормаОтсутствует = ТекущиеДанные.ФормаОтсутствует;
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ЗаполнитьСвойстваТестируемойФормы(ТекущиеДанные)
	...
	АдресДанныхДляГенерацииАвтотестов = ПоместитьВоВременноеХранилище(Неопределено, ЭтотОбъект.УникальныйИдентификатор);
	МФИнструментыРазработчикаКлиент.СформироватьДанныеДляГенерацииАвтотестов(
...
// Функция генерации текста модуля с тестами
&НаСервереБезКонтекста
Функция ТекстМодуляТестированияПоМакету(ИмяФормы, ИмяМакета, Шаблоны, АдресДанныхДляГенерацииАвтотестов, ЯзыкГенерации)
	...
	СоздатьИсполняемыеСценарииПроверкиРеквизитов(
		ДанныеДляГенерацииАвтотестов,
		ИсполняемыеСценарии, 
		Шаблоны,
		ЯзыкГенерации
	);
	СоздатьИсполняемыеСценарииПроверкиКоманд(
		ДанныеДляГенерацииАвтотестов,
		ИсполняемыеСценарии, 
		Шаблоны,
		ЯзыкГенерации
...
&НаСервереБезКонтекста
Процедура СоздатьИсполняемыеСценарииПроверкиРеквизитов(ДанныеДляГенерацииАвтотестов, ИсполняемыеСценарии, Шаблоны, ЯзыкГенерации)
	ОписаниеМодификацийРеквизитов = ДанныеДляГенерацииАвтотестов.Реквизиты;
	...
	Для Каждого Элемент Из ОписаниеМодификацийРеквизитов Цикл
		ИмяРеквизита = Элемент.Значение.ИмяРеквизита;
		РодительРеквизита = СокрЛП(Элемент.Значение.Родитель);
		Если Не ПустаяСтрока(РодительРеквизита) Тогда
			ИмяРеквизита = СтрШаблон("%1.%2", РодительРеквизита, ИмяРеквизита);
		КонецЕсли;
		...
		Тесты = СтрЗаменить(Тесты, "&&ИмяРеквизита", ИмяРеквизита);
	КонецЦикла;
	Тесты = СокрП(Тесты);
...
&НаСервереБезКонтекста
Процедура СоздатьИсполняемыеСценарииПроверкиКоманд(ДанныеДляГенерацииАвтотестов, ИсполняемыеСценарии, Шаблоны, ЯзыкГенерации)
	ОписаниеМодификацийКоманд = ДанныеДляГенерацииАвтотестов.Команды;
	Если ОписаниеМодификацийКоманд.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	Тесты = "";
	Для Каждого Команда Из ОписаниеМодификацийКоманд Цикл
		ИмяКоманды = Команда.Значение.Имя;
		
		// Добавляем тест проверки существования команды
		ДобавитьТест(
			Тесты, 
			"КомандаФормыСуществует",
			"Команда с именем """"&&ИмяКоманды"""" существует",
			"""&&ИмяКоманды""",
			Шаблоны
		);
		
		Для Каждого СвойствоКоманды Из Команда.Значение.ПредставленияСвойствОбъекта Цикл
			ИмяСвойства= СвойствоКоманды.Ключ;
			ЗначениеСвойстваКомандыФормы = ЗначениеСвойстваКомандыФормы(
				Команда,
				СвойствоКоманды,
				ДанныеДляГенерацииАвтотестов,
				ЯзыкГенерации
			...
			ДобавитьТест(
				Тесты, 
				"СвойствоКомандыФормыРавно",
				"Свойство """"&&ИмяСвойстваКомандыФормы"""" команды с именем """"&&ИмяКоманды"""" равно """"&&ПредставлениеЗначенияСвойства""""",
				"""&&ИмяКоманды"",""&&ИмяСвойстваКомандыФормы"",""&&ЗначениеСвойстваКомандыФормы""",
				Шаблоны
			);
			Тесты = СтрЗаменить(
				Тесты,
				"&&ИмяСвойстваКомандыФормы",
				ИмяСвойства
			);
			Тесты = СтрЗаменить(
				Тесты,
				"&&ПредставлениеЗначенияСвойства",
				СвойствоКоманды.Значение
			);
			Тесты = СтрЗаменить(
				Тесты,
				"&&ЗначениеСвойстваКомандыФормы",
				ЗначениеСвойстваКомандыФормы
			);
		КонецЦикла;
		
		Тесты = СтрЗаменить(Тесты, "&&ИмяКоманды", ИмяКоманды);
	КонецЦикла;
	Тесты = СокрП(Тесты);
	
	Шаблон = Шаблоны.ИсполняемыеСценарии;
	ТекущиеИсполняемыеСценарии = СтрЗаменить(Шаблон, "&&ИмяТестовогоНабора", Шаблоны.ИмяТестовогоНабораКоманды);
	ТекущиеИсполняемыеСценарии = СтрЗаменить(ТекущиеИсполняемыеСценарии, "&&Тесты", Тесты);
	ТекущиеИсполняемыеСценарии = СтрЗаменить(
		ТекущиеИсполняемыеСценарии,
		"&&ИмяПроцедурыПеред",
		"ПодготовкаДанныхДляТестированияКоманд"
	);
...
&НаСервереБезКонтекста
Процедура СоздатьИсполняемыеСценарииПроверкиЭлементовФормы(ДанныеДляГенерацииАвтотестов, ИсполняемыеСценарии, Шаблоны, ЯзыкГенерации)
	ОписаниеМодификацийЭлементов = ДанныеДляГенерацииАвтотестов.Элементы;
	Если ОписаниеМодификацийЭлементов.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	СоответствиеТиповЭлементовФормыСтроке = МФИнструментыРазработчикаКлиентСервер.СоответствиеТиповЭлементовФормыСтроке();
	...
	Для Каждого Элемент Из ОписаниеМодификацийЭлементов Цикл
		...
		ДобавитьТест(
			...
			"Элемент формы с именем """"&&ИмяЭлементаФормы"""" существует",
			"""&&ИмяЭлементаФормы""",
			Шаблоны
		);
		...
		Для Каждого СвойствоЭлемента Из Элемент.Значение.ПредставленияСвойствОбъекта Цикл
			ИмяСвойства= СвойствоЭлемента.Ключ;
			ЗначениеСвойстваЭлементаФормы = ЗначениеСвойстваЭлементаФормы(
				Элемент,
				СвойствоЭлемента,
				ДанныеДляГенерацииАвтотестов,
				ЯзыкГенерации
			);
			ОписаниеСвойства = МФОбъектыФормыПовтИсп
				.ОписанияСвойствЭлементовФормы()
				.Получить(Элемент.Значение.ТипЭлемента)
			;
			ИдентификаторСвойства = ОписаниеСвойства
				.СоответствиеСвойств
				.Получить(СвойствоЭлемента.Ключ)
			;
			ВозможныеТипы = ОписаниеСвойства.Свойства.Получить(ИдентификаторСвойства).ТипыСвойства;
			ТипСвойства = Неопределено;
			Для Каждого ВозможныйТип Из ВозможныеТипы Цикл
				Если 
					ВозможныйТип.ИмяПоля = Неопределено 
					Или (
						Элемент.Значение.СвойстваОбъекта.Свойство(ВозможныйТип.ИмяПоля)
						И Элемент.Значение.СвойстваОбъекта[ВозможныйТип.ИмяПоля] = ВозможныйТип.ЗначениеПоля
					)
					Тогда
						ТипСвойства = ВозможныйТип.Тип;
					Прервать;
				КонецЕсли;
			КонецЦикла;
			Если ТипСвойства = МФПовтИсп.ТипСвязиПараметровВыбора() Тогда
				ЗначениеСвойстваЭлементаФормы = СтрШаблон("СвязиПараметровВыбораИзСтроки(""""%1"""")", ЗначениеСвойстваЭлементаФормы);
			КонецЕсли;
...
#Область ЗначениеСвойстваКомандыФормы

&НаСервереБезКонтекста
Функция ЗначениеСвойстваКомандыФормы(КомандаФормы, Свойство, ДанныеДляГенерацииАвтотестов, ЯзыкГенерации)
	Результат = "";
	
	ОписанияСвойствКомандыФормы = МФОбъектыФормыПовтИсп.ОписанияСвойствКомандФормы();
	ИмяСвойства= Свойство.Ключ;
	ЗначениеСвойства = КомандаФормы.Значение.СвойстваОбъекта[
		ОписанияСвойствКомандыФормы.СоответствиеСвойств.Получить(ИмяСвойства)
	];
	
	Результат = ЗначениеСвойстваОбъекта(Свойство, ЗначениеСвойства, ДанныеДляГенерацииАвтотестов, ЯзыкГенерации);
...
&НаСервереБезКонтекста
Функция ЗначениеСвойстваЭлементаФормы(ЭлементФормы, Свойство, ДанныеДляГенерацииАвтотестов, ЯзыкГенерации)
	Результат = "";
	
	ОписаниеСвойствЭлементаФормы = МФОбъектыФормыПовтИсп
		.ОписанияСвойствЭлементовФормы()
		.Получить(ЭлементФормы.Значение.ТипЭлемента)
	;
	ИмяСвойства= Свойство.Ключ;
	ЗначениеСвойства = ЭлементФормы.Значение.СвойстваОбъекта[
...
&НаСервереБезКонтекста
Функция ЗначениеСвойстваОбъекта(Свойство, ЗначениеСвойства, ДанныеДляГенерацииАвтотестов, ЯзыкГенерации)
	Результат = "";
	
	ТипЗначенияСвойства = ТипЗнч(ЗначениеСвойства);
	Если ТипЗначенияСвойства = Тип("Цвет") Тогда
		Результат = ЗначениеСвойстваТипаЦвет(ЗначениеСвойства);
	ИначеЕсли ТипЗначенияСвойства = Тип("Строка") Тогда
		Результат = СтрШаблон(
		"""""%1""""",
		СтрЗаменить(ЗначениеСвойства, """", """""")
		);
	ИначеЕсли ТипЗначенияСвойства = Тип("Картинка") Тогда
		ОписаниеЗначения = Неопределено;
		Если Не ДанныеДляГенерацииАвтотестов.Картинки.Свойство(ВРег(Свойство.Значение), ОписаниеЗначения) Тогда
			ТекстИсключения = СтрШаблон(
				НСтр("ru='Ошибка получения значения ""%1"" типа ""Картинка""'"),
				Свойство.Значение
			);
			
			ВызватьИсключение ТекстИсключения;
		КонецЕсли;
		Если МФОбработчикСтрокиКартинка.ЭтоБиблиотекаКартинок(ОписаниеЗначения.Значение.Источник) Тогда
			Результат = "БиблиотекаКартинок." + ОписаниеЗначения.Значение.ИмяВИсточнике;
		Иначе
			Результат = "";
		КонецЕсли;
	ИначеЕсли ТипЗначенияСвойства = Тип("ФиксированныйМассив") Тогда
		// TODO 2026-03-16: сделать формирование теста на СвязиПараметровВыбора
		Состав = Новый Массив;
		Для Каждого ЭлементКоллекции Из ЗначениеСвойства Цикл
			Если ТипЗнч(ЭлементКоллекции) = Тип("СвязьПараметраВыбора") Тогда
				Состав.Добавить(СтрШаблон(
					"%1;%2;%3",
					ЭлементКоллекции.Имя,
					ЭлементКоллекции.ПутьКДанным,
					ЭлементКоллекции.ИзменениеЗначения
				));
			КонецЕсли;
		КонецЦикла;
...
&НаСервереБезКонтекста
Функция ЗначениеСвойстваТипаЦвет(ЗначениеСвойства)
	Результат = "";
...
Процедура ПослеВыбораКаталогаДляСохраненияМодулей(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	ЭтотОбъект.СохраняемыеМодули = Новый Структура;
	Для Каждого СтрокаТаблицы Из ТестируемыеФормы Цикл
		Если ПустаяСтрока(СтрокаТаблицы.ИмяМодуля) Или ПустаяСтрока(СтрокаТаблицы.ТекстМодуля) Тогда
			Продолжить;
		КонецЕсли;
		
		ЭтотОбъект.СохраняемыеМодули.Вставить(СтрокаТаблицы.ИмяМодуля, Новый Структура(
	...
	Для Каждого Элемент Из ЭтотОбъект.СохраняемыеМодули Цикл
		ОписаниеОповещения = Новый ОписаниеОповещения(
...
Процедура ПослеСохраненияМодулей(Результат, ДополнительныеПараметры) Экспорт
	...
	// записи всех модулей
	ЭтотОбъект.СохраняемыеМодули[ДополнительныеПараметры].ЗаданиеВыполнено = Истина;
	ЭтотОбъект.СохраняемыеМодули[ДополнительныеПараметры].ФайлСохранен = Результат;
...
&НаКлиенте
Асинх Процедура ПослеУстановкиСвойствГенерацииРасширения(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	Если РезультатЗакрытия = Неопределено Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого ПараметрИнициализации Из ПараметрыИнициализации Цикл
		Если ИменаПараметров.Найти(ПараметрИнициализации.Ключ) = Неопределено Тогда
			Продолжить;
		ИначеЕсли РезультатЗакрытия.Свойство(ПараметрИнициализации.Ключ) Тогда
			ПараметрыИнициализации[ПараметрИнициализации.Ключ] = РезультатЗакрытия[ПараметрИнициализации.Ключ];
		Иначе
			ПараметрыИнициализации[ПараметрИнициализации.Ключ] = ЭтотОбъект[ПараметрИнициализации.Ключ];
		КонецЕсли;
	КонецЦикла;
	ПолныйПутьКФайлуРасширения = РезультатЗакрытия.ПолныйПутьКФайлуРасширения;
	
	ГенераторРасширения.Инициализировать(ПараметрыИнициализации);
	ФормироватьМодулиЯдраТестирования = РезультатЗакрытия.ФормироватьМодулиЯдраТестирования;
	Если РезультатЗакрытия.РежимГенерации = МФИнструментыРазработчикаКлиент.РежимГенерацииРасширенияСоздание() Тогда
		ГенераторРасширения.Открыть();
		ПодключитьОбработчикОжидания("СоздатьРасширениеСТестами_Выполнение", 0.1, Истина);
	Иначе
		ТекстИсключения = СтрШаблон(
			НСтр("ru='Неизвестный режим генерации расширения с тестами: %1'"),
			РезультатЗакрытия.РежимГенерации
		);
...
&НаКлиенте
Асинх Процедура СоздатьРасширениеСТестами_Выполнение()
	Модули = Новый Массив;
	Для Каждого СтрокаТаблицы Из ТестируемыеФормы Цикл
		Если Не (ПустаяСтрока(СтрокаТаблицы.ИмяМодуля) Или ПустаяСтрока(СтрокаТаблицы.ТекстМодуля)) Тогда
			Модули.Добавить(Новый Структура(
				"ИмяМодуля, ТекстМодуля, ИмяФормы",
				СтрокаТаблицы.ИмяМодуля,
				СтрокаТаблицы.ТекстМодуля,
				СтрокаТаблицы.ИмяФормы
			));
		КонецЕсли;
	КонецЦикла;
	
	Ждать ГенераторРасширения.СоздатьРасширениеСТестами(ПолныйПутьКФайлуРасширения, Модули, ФормироватьМодулиЯдраТестирования);
КонецПроцедуры

&НаКлиенте
Асинх Процедура ОбновитьРасширениеСТестами_Выполнение()
	Модули = Новый Массив;
	Для Каждого СтрокаТаблицы Из ТестируемыеФормы Цикл
		Если Не (ПустаяСтрока(СтрокаТаблицы.ИмяМодуля) Или ПустаяСтрока(СтрокаТаблицы.ТекстМодуля)) Тогда
			Модули.Добавить(Новый Структура(
	...
	Ждать ГенераторРасширения.ОбновитьРасширениеСТестами(ПолныйПутьКФайлуРасширения, Модули);
КонецПроцедуры

&НаКлиенте
Функция ФормаОтсутствует(ПроверяемоеИмяФормы)
	Результат = Ложь;
	Попытка
		ПолучитьФорму(ПроверяемоеИмяФормы);
	Исключение
		Результат = Истина;
	КонецПопытки;
...
&НаКлиенте
Асинх Процедура ЗагрузитьИзРасширения_Выполнение()
	Ждать ГенераторРасширения.ПолучитьДополнительныеСвойстваРасширенияСТестами(ПолныйПутьКФайлуРасширения);
	//Тесты = Ждать ГенераторРасширения.ПрочитатьТестыИзРасширения(ПолныйПутьКФайлуРасширения);
	//Ждать ГенераторРасширения.ДополнитьСвойстваРасширения(СвойстваРасширения, ИмяФайлаРасширения);
	//Если СвойстваРасширения.Тесты.Количество() = 0 Тогда
	//	ТекстСообщения = СтрШаблон(
	//		НСтр("ru='В расширении ""%1"" отсутствует информация о тестах'"),
	//		ИмяФайлаРасширения
	//	);
	//	Ждать ПредупреждениеАсинх(ТекстСообщения, 30, НСтр("ru='Предупреждение'"));
	//	
	//	Возврат;
	//КонецЕсли;
	//
	//ТестируемыеФормы.Очистить();
	//Для Каждого Элемент Из СвойстваРасширения.Тесты Цикл
	//	НоваяСтрока = ТестируемыеФормы.Добавить();
	//	НоваяСтрока.ИмяФормы = Элемент.ИмяФормы;
	//	НоваяСтрока.ФормаОтсутствует = ФормаОтсутствует(Элемент.ИмяФормы);
	//	
	//	
	//	ЗаполнитьСвойстваТестируемойФормы(НоваяСтрока);
	//КонецЦикла;
...
&НаКлиенте
Процедура Обработка_УспешноеЗавершениеСозданияРасширенияСТестами(Параметр)
	ВерсияРасширения = Параметр.ВерсияРасширения;
	ИмяРасширения = Параметр.ИмяРасширения;
	ГенераторРасширения.ЗакрытьФорму();
	ПоказатьПредупреждение(
		,
		НСтр("ru='Создание расширения завершено успешно!'"),
		30
	);
КонецПроцедуры

&НаКлиенте
Процедура Обработка_ОшибкаСозданияРасширенияСТестами(Параметр)
	ПолныйПутьКФайлуРасширения = "";
	ГенераторРасширения.ЗакрытьФорму();
	
	ВызватьИсключение Параметр;
КонецПроцедуры

&НаКлиенте
Процедура Обработка_УспешноеЧтениеДополнительныхСвойствРасширенияСТестами(Параметр)
	ГенераторРасширения.ЗакрытьФорму();
	
	ТестируемыеФормы.Очистить();
	Для Каждого ТестИзРасширения Из Параметр.Тесты Цикл
		НоваяСтрока = ТестируемыеФормы.Добавить();
		НоваяСтрока.ИмяФормы = ТестИзРасширения.ИмяФормы;
		НоваяСтрока.ФормаОтсутствует = ФормаОтсутствует(НоваяСтрока.ИмяФормы);
		ЗаполнитьСвойстваТестируемойФормы(НоваяСтрока);
	КонецЦикла;
	ПолныйПутьКФайлуРасширения = Параметр.ПолныйПутьКФайлуРасширения;
	Если Параметр.Версия = Версия Тогда
		ФормироватьМодулиЯдраТестирования = Ложь;
	КонецЕсли;
	...
	ПоказатьПредупреждение(
		,
		НСтр("ru='Тесты из расширения прочитаны успешно!'"),
		30
	);
КонецПроцедуры

&НаКлиенте
Процедура Обработка_ОшибкаЧтенияДополнительныхСвойствРасширенияСТестами(Параметр)
	ГенераторРасширения.ЗакрытьФорму();
...
&НаКлиенте
Асинх Функция ВопросОчисткиСпискаТестов()
	...
	Результат = Ждать ВопросАсинх(
		НСтр("ru='Перед загрузкой текущий список тестируемых форм будет очищен. Продолжить?'"),
		РежимДиалогаВопрос.ДаНет,
		30,
		КодВозвратаДиалога.Нет,
		НСтр("ru='Вопрос'"),
...
Функция ФормыИзКэша()
	...
	Кэш = РегистрыСведений.МФМакеты.КэшФорм("ИмяФормы", РегистрыСведений.МФМакеты.РежимОтбораАктуальныеФормы());
	Для Каждого ЭлементКоллекции Из Кэш Цикл
