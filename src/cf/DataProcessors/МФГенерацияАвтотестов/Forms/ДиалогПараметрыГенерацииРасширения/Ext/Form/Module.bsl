...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	Если ИмяСобытия = "УспешноеЗавершениеПроверкиПодключения" Тогда
	...
	ИначеЕсли ИмяСобытия = "ОшибкаПроверкиПодключения" Тогда
		ГенераторРасширения.ЗакрытьФорму();
...
Асинх Процедура ПроверитьСоединение(Команда)
	...
	ПодключитьОбработчикОжидания("ПроверитьСоединение_Выполнение", 0.1, Истина);
КонецПроцедуры

&НаКлиенте
Процедура ЗаполнитьПутьКФайлуЗапуска1СПоУмолчанию(Команда)
...
&НаКлиенте
Асинх Функция ПроверитьСоединение_Выполнение()
