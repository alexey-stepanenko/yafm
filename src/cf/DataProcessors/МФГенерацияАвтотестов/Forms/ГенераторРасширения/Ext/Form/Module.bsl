...
&НаКлиенте
Асинх Функция СоздатьРасширениеСТестами(_ПолныйПутьКФайлуРасширения, МодулиТестирования, ФормироватьМодулиЯдраТестирования) Экспорт
	Данные = НовыйДанныеДляСозданияРасширенияСТестами();
	Данные.МодулиТестирования = МодулиТестирования;
	Данные.ФормироватьМодулиЯдраТестирования = ФормироватьМодулиЯдраТестирования;
...
Функция НовыйДанныеДляСозданияРасширенияСТестами()
	...
	Результат.Вставить("ФормироватьМодулиЯдраТестирования", Неопределено);
...
Асинх Функция СоздатьРасширениеСТестами_ФормированиеИсходниковРасширения()
	Попытка
		...
		// TODO 2026-01-04: Выяснить что нужно удалить
		// TODO 2025-12-05: Удалить
		...
		// Если режим формирования модулей ядра расширения снят, то удаляем модули,
		// входящие в подсистему МФТестирование
		Если Не Данные.ФормироватьМодулиЯдраТестирования Тогда
			ИмяПодсистемы = "МФТестирование";
			ПутьКФайлуОписанияПодсистемы = ПутьКОбъекту(
				Данные.ПутьККаталогуИсходников,
				СтрШаблон("Subsystems,%1.xml", ИмяПодсистемы)
			);
			ТекстовыйДокумент = Новый ТекстовыйДокумент;
			Ждать ТекстовыйДокумент.ПрочитатьАсинх(ПутьКФайлуОписанияПодсистемы);
			НачалоСтроки = "<xr:Item xsi:type=""xr:MDObjectRef"">CommonModule.";
			КонецСтроки = "</xr:Item>";
			ИменаМодулей = Новый Массив;
			Для Сч = 1 По ТекстовыйДокумент.КоличествоСтрок() Цикл
				АнализируемаяСтрока = СокрЛП(ТекстовыйДокумент.ПолучитьСтроку(Сч));
				Если Лев(АнализируемаяСтрока, СтрДлина(НачалоСтроки)) <> НачалоСтроки Тогда
					Продолжить;
				КонецЕсли;
				ИмяМодуля = Сред(
					АнализируемаяСтрока, 
					СтрДлина(НачалоСтроки) + 1,
					СтрДлина(АнализируемаяСтрока) - СтрДлина(НачалоСтроки) - СтрДлина(КонецСтроки)
				);
				ИменаМодулей.Добавить(ИмяМодуля);
			КонецЦикла;
			
			ПутьКФайлуConfiguration = ПутьКОбъекту(Данные.ПутьККаталогуИсходников, "Configuration.xml");
			ТекстовыйДокумент = Новый ТекстовыйДокумент;
			Ждать ТекстовыйДокумент.ПрочитатьАсинх(ПутьКФайлуConfiguration);
			ОписаниеКонфигурации = ТекстовыйДокумент.ПолучитьТекст();
			ОписаниеКонфигурации = СтрЗаменить(ОписаниеКонфигурации, СтрШаблон("<Subsystem>%1</Subsystem>", ИмяПодсистемы), "");
			Для Каждого ИмяМодуля Из ИменаМодулей Цикл
				ОписаниеКонфигурации = СтрЗаменить(
					ОписаниеКонфигурации,
					СтрШаблон("<CommonModule>%1</CommonModule>", ИмяМодуля),
					""
				);
			КонецЦикла;
			ТекстовыйДокумент.УстановитьТекст(ОписаниеКонфигурации);
			Ждать ТекстовыйДокумент.ЗаписатьАсинх(ПутьКФайлуConfiguration);
		КонецЕсли;
		
		// Формируем манифест
		Данные.Манифест = НовыйМанифест();
		Данные.Манифест.core_testing_modules = Данные.ФормироватьМодулиЯдраТестирования;
...
Асинх Функция СоздатьРасширениеСТестами_ФормированиеШаблонаОписанияМодуляСТестом()
	Попытка
		...
		// * Ищем файл шаблона описания общего модуля с тестом
		Данные.ПутьККаталогуОбщихМодулей = ПутьКОбъекту(Данные.ПутьККаталогуИсходников, "CommonModules");
		ПолноеИмяИсходногоФайла = ПутьКОбъекту(Данные.ПутьККаталогуОбщихМодулей, "МФШаблонМодуляСТестом.xml");
		...
		ЗаменитьВхожденияНаСтроку(Данные.ТекстШаблонаОписанияМодуля, "<CommonModule uuid=""", "<CommonModule uuid=""&&GUID0&&"">");
		ЗаменитьВхожденияНаСтроку(Данные.ТекстШаблонаОписанияМодуля, "<Name>", "<Name>&&ModuleName&&</Name>");
		ЗаменитьВхожденияНаСтроку(Данные.ТекстШаблонаОписанияМодуля, "<v8:content>", "<v8:content>&&ModuleName&&</v8:content>");
		...
		ЗаменитьВхожденияНаСтроку(ТекстШаблонаОписанияКонфигурации, "<Configuration uuid=""", "<Configuration uuid=""&&GUID0&&"">");
		ЗаменитьВхожденияНаСтроку(ТекстШаблонаОписанияКонфигурации, "<Name>", "<Name>&&Name&&</Name>");
		ЗаменитьВхожденияНаСтроку(ТекстШаблонаОписанияКонфигурации, "<v8:content>", "<v8:content>&&Name&&</v8:content>");
		ЗаменитьВхожденияНаСтроку(ТекстШаблонаОписанияКонфигурации, "<Version>", "<Version>&&Version&&</Version>");
...
Асинх Функция СоздатьРасширениеСТестами_ДополнительнаяОбработкаИсходников()
	Попытка
		...
		// Удаляем модуль МФШаблонМодуляСТестом из расширения
		КорректируемыеФайлы = Новый Массив;
		КорректируемыеФайлы.Добавить(ПутьКОбъекту(Данные.ПутьККаталогуИсходников, "Configuration.xml"));
		КорректируемыеФайлы.Добавить(ПутьКОбъекту(Данные.ПутьККаталогуИсходников, "Subsystems,МФТестирование.xml"));
		Для Каждого КорректируемыйФайл Из КорректируемыеФайлы Цикл
			ТекстовыйДокумент = Новый ТекстовыйДокумент;
			Ждать ТекстовыйДокумент.ПрочитатьАсинх(КорректируемыйФайл);
			ТекстМодуля = ТекстовыйДокумент.ПолучитьТекст();
			ЗаменитьВхожденияНаСтроку(ТекстМодуля, "<CommonModule>МФШаблонМодуляСТестом", "");
			ЗаменитьВхожденияНаСтроку(ТекстМодуля, "<xr:Item xsi:type=""xr:MDObjectRef"">CommonModule.МФШаблонМодуляСТестом", "");
			ТекстовыйДокумент.УстановитьТекст(ТекстМодуля);
			Ждать ТекстовыйДокумент.ЗаписатьАсинх(КорректируемыйФайл);
		КонецЦикла;
...
&НаКлиенте
Процедура ЗаменитьВхожденияНаСтроку(ТекстШаблона, СтрокаПоиска, СтрокаЗамены)
	ТекстовыйДокумент = Новый ТекстовыйДокумент;
...
// TODO 2025-07-04: Реализовать удаление из информационной базы расширения с именем ИмяРасширения
&НаКлиенте
...
&НаКлиенте
Асинх Функция УдалитьРасширение()
// УДАЛИТЬ ИЗ РЕЛИЗА: НАЧАЛО БЛОКА
	ДобавитьВЛог(
		"Информация",
		ИмяРасширения,
		СтрШаблон(
			"Попытка удаления расширения ""%1"" из временной информационной базы ""%2""",
			ИмяРасширения,
			СтрокаСоединенияКСуществующейИБ
		)
	);
// УДАЛИТЬ ИЗ РЕЛИЗА: КОНЕЦ БЛОКА
	...
	Ждать ВыполнитьКомандуКонфигуратора(
		СоставСтрокиЗапуска,
		"Ошибка удаления расширения из временной информационной базы"
	);
КонецФункции
...
Функция НовыйМанифест()
	...
	Результат.Вставить("core_testing_modules", Данные.ФормироватьМодулиЯдраТестирования);
