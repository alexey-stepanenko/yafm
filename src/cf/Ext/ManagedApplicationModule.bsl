...
// РЕЖИМ РАЗРАБОТКИ: &После("ПриНачалеРаботыСистемы")
// РЕЖИМ РАЗРАБОТКИ: Процедура Разработка_ПриНачалеРаботыСистемы()
...
// РЕЖИМ РАЗРАБОТКИ: НАЧАЛО БЛОКА
	...
	Если МФИнструментыРазработчика.ВозможенЗапуск(Сообщения) Тогда
		МФИнструментыРазработчикаКлиент.ПродолжитьЗапускИнструментовРазработчика(Неопределено);
	Иначе
		Причина = "";
		Если Сообщения.Количество() = 1 Тогда
			Причина = НСтр("ru=', т.к. '") + НРег(Лев(Сообщения[0], 1)) + Прав(Сообщения[0], СтрДлина(Сообщения[0]) - 1);
		Иначе
			СоставПричин = Новый Массив;
			Для Каждого ЭлементКоллекции Из Сообщения Цикл
				СоставПричин.Добавить("* " + ЭлементКоллекции);
			КонецЦикла;
			Причина = " по причинам:" + Символы.ПС + СтрСоединить(СоставПричин, Символы.ПС);
		КонецЕсли;
		ТекстПредупреждения = НСтр("ru='Функциональность инструментов разработчика может быть нарушена'") + Причина;
		ОписаниеОповещения = Новый ОписаниеОповещения(
			"ПродолжитьЗапускИнструментовРазработчика",
			МФИнструментыРазработчикаКлиент
		);
