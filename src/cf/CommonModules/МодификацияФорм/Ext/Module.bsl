...
#Если Не МобильныйКлиент И Не МобильноеПриложениеКлиент И Не МобильноеПриложениеСервер И Не МобильныйАвтономныйСервер Тогда
...
Процедура МодифицироватьФорму(Форма, Отказ, ИмяМакетаОписанияМодификаций, ГенераторПротокола = Неопределено) Экспорт
	...
	Попытка
		...
		МФСлужебный.ПередМодификациейФормы(Форма, ОписаниеМодификаций);
		
		ДополнительноОбработатьРеквизиты(ОписаниеМодификаций, Протокол);
		ДополнительноОбработатьКоманды(ОписаниеМодификаций, Протокол);
	...
	МФСлужебный.ПослеМодификацииФормы(Форма, ОписаниеМодификаций);
КонецПроцедуры
...
#Область ДополнительнаяОбработкаКоманд

Процедура ДополнительноОбработатьКоманды(ОписаниеМодификаций, Протокол)
	ОписаниеМодификацийКоманд = ОписаниеМодификаций.Команды;
	ОписаниеСвойствКоманды = МФОбъектыФормыПовтИсп.ОписанияСвойствКомандФормы();
	Для Каждого ЭлементКоллекции Из ОписаниеМодификацийКоманд Цикл
		ОписаниеКоманды = ЭлементКоллекции.Значение;
		ИмяКоманды = ОписаниеКоманды.Имя;
		
		// TODO 2026-03-01: Удалить блоки проверки после перехода на релиз 1.2.xxx.xxx
		// {{
		Если Не ПустаяСтрока(ОписаниеКоманды.Заголовок) Тогда
			Протокол.ГенераторПротокола.ДобавитьСтрокуВПротокол(Протокол, СтрШаблон(
				НСтр("ru='Обнаружен устаревший модификатор ""%1"" для команды ""%2""'"),
				"ЗАГОЛОВОККОМАНДЫ",
				ИмяКоманды
			));
		КонецЕсли;
		Если Не ПустаяСтрока(ОписаниеКоманды.Действие) Тогда
			Протокол.ГенераторПротокола.ДобавитьСтрокуВПротокол(Протокол, СтрШаблон(
				НСтр("ru='Обнаружен устаревший модификатор ""%1"" для команды ""%2""'"),
				"ДЕЙСТВИЕКОМАНДЫ",
				ИмяКоманды
		...
		КонецЕсли;
		// }}
		ЗначенияСвойствКоманды = ЗначенияСвойствКоманды(ОписаниеКоманды, ОписаниеМодификаций, Протокол);
		Для Каждого ЗначениеСвойстваКоманды Из ЗначенияСвойствКоманды Цикл
			ОписаниеКоманды.СвойстваОбъекта.Вставить(ЗначениеСвойстваКоманды.Ключ, ЗначениеСвойстваКоманды.Значение);
		КонецЦикла;
	КонецЦикла;
КонецПроцедуры

Функция ЗначенияСвойствКоманды(ОписаниеКоманды, ОписаниеМодификаций, Протокол)
	Результат = Новый Структура;
	
	Для Каждого ЭлементКоллекции Из ОписаниеКоманды.СвойстваОбъекта Цикл
	...
	СвойстваКОбработке = Новый Структура(Новый ФиксированнаяСтруктура(ОписаниеКоманды.ПредставленияСвойствОбъекта));
	Для Каждого СвойствоКОбработке Из СвойстваКОбработке Цикл
		ИдентификаторСвойства = ОписанияСвойствКомандыФормы.СоответствиеСвойств.Получить(СвойствоКОбработке.Ключ);
		ПредставлениеЗначенияСвойства = СвойствоКОбработке.Значение;
		Попытка
			ОписаниеСвойства = ОписанияСвойствКомандыФормы.Свойства.Получить(ИдентификаторСвойства);
			...
			Для Каждого Элемент Из ВозможныеТипыСвойства Цикл
				Если 
					...
					Или (
						Результат.Свойство(Элемент.ИмяПоля)
						И Результат[Элемент.ИмяПоля] = Элемент.ЗначениеПоля
					)
				Тогда
		...
		Исключение
			...
			КонецЕсли;
		КонецПопытки;
		Результат.Вставить(ИдентификаторСвойства, ЗначениеСвойства);
	КонецЦикла;
...
Процедура ДополнительноОбработатьЭлементы(ОписаниеМодификаций, Протокол, Форма)
	...
	ПроверитьОбработчики(ОписаниеМодификаций, Протокол);
КонецПроцедуры
...
Процедура ЗаполнитьОбязательныеСвойстваСуществующихЭлементов(ЭлементыФормы, ОписанияЭлементов)
	Для Каждого ЭлементКоллекции Из ОписанияЭлементов Цикл
		...
		Если ЭлементФормы = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		ОписаниеТипаЭлемента = МФОбъектыФормыПовтИсп
		...
		Для Каждого Элемент Из ОписаниеТипаЭлемента.ОбязательныеСвойства Цикл
			...
			ОписаниеЭлемента.СвойстваОбъекта.Вставить(ИдентификаторСвойства, ЭлементФормы[ИмяСвойства]);
		КонецЦикла;
	КонецЦикла;
КонецПроцедуры
...
Процедура СформироватьЗначенияСвойствЭлементов(ОписаниеМодификаций, Протокол)
	...
	Для Каждого ЭлементКоллекции Из ОписанияЭлементов Цикл
		...
		Для Каждого Элемент Из ЗначенияСвойствЭлемента Цикл
			...
			ОписаниеЭлемента.СвойстваОбъекта.Вставить(ИдентификаторСвойства, ЗначениеСвойства);
		КонецЦикла;
	КонецЦикла;
КонецПроцедуры
...
Функция ЗначенияСвойствЭлемента(ОписаниеЭлемента, ОписаниеМодификаций, Протокол)
	...
	Для Каждого ЭлементКоллекции Из ОписаниеЭлемента.СвойстваОбъекта Цикл
		Результат.Вставить(ЭлементКоллекции.Ключ, ЭлементКоллекции.Значение);
	КонецЦикла;
	...
	ОписаниеСвойствЭлементаФормы = МФОбъектыФормыПовтИсп
		...
		.Получить(ОписаниеЭлемента.ТипЭлемента)
	;
	СвойстваКОбработке = Новый Структура(Новый ФиксированнаяСтруктура(ОписаниеЭлемента.ПредставленияСвойствОбъекта));
...
Процедура ПроверитьОбработчики(ОписаниеМодификаций, Протокол)
	Для Каждого ЭлементКоллекции Из ОписаниеМодификаций.Элементы Цикл
		ЭлементФормы = ЭлементКоллекции.Значение;
		Если ЭлементФормы.ОбработчикиЭлемента.Количество() = 0 Тогда
			Продолжить;
		КонецЕсли;
		ОписаниеЭлемента = МФОбъектыФормыПовтИсп
			.ОписанияСвойствЭлементовФормы()
			.Получить(ЭлементФормы.ТипЭлемента)
		;
		ИдентификаторыУдаляемыхОбработчиков = Новый Массив;
		Для Каждого ОбработчикЭлемента Из ЭлементФормы.ОбработчикиЭлемента Цикл
			ИдентификаторСобытия = ОписаниеЭлемента.СоответствиеСвойств.Получить(ОбработчикЭлемента.Ключ);
			Если ИдентификаторСобытия = Неопределено Тогда
				ИдентификаторыУдаляемыхОбработчиков.Добавить(ОбработчикЭлемента.Ключ);
				Продолжить;
			КонецЕсли;
			ОбработчикиЭлемента = ОписаниеЭлемента.Разделы.Получить(МФПовтИсп.РазделСобытия());
			Если ОбработчикиЭлемента.Найти(ИдентификаторСобытия) = Неопределено Тогда
				ИдентификаторыУдаляемыхОбработчиков.Добавить(ОбработчикЭлемента.Ключ);
				Продолжить;
			КонецЕсли;
			ОписаниеОбработчика = ОписаниеЭлемента.Свойства.Получить(ИдентификаторСобытия);
			ВозможныеТипы = ОписаниеОбработчика.ТипыСвойства;
			НашлиОбработчик = Ложь;
			Для Каждого Элемент Из ВозможныеТипы Цикл
				Если 
					Элемент.ИмяПоля = Неопределено 
					Или (
						ЭлементФормы.СвойстваОбъекта.Свойство(Элемент.ИмяПоля)
						И ЭлементФормы.СвойстваОбъекта[Элемент.ИмяПоля] = Элемент.ЗначениеПоля
					)
				Тогда
					НашлиОбработчик = Истина;
					Прервать;
				КонецЕсли;
			КонецЦикла;
			Если Не НашлиОбработчик Тогда
				ИдентификаторыУдаляемыхОбработчиков.Добавить(ОбработчикЭлемента.Ключ);
				Продолжить;
			КонецЕсли;
		КонецЦикла;
		Для Каждого ИдентификаторУдаляемогоОбработчика Из ИдентификаторыУдаляемыхОбработчиков Цикл
			СтрокаПротокола = СтрШаблон(
				НСтр("ru='Элемент формы ""%1"" не имеет события ""%2""'"),
				ЭлементФормы.ИмяЭлемента,
				ЭлементФормы.ОбработчикиЭлемента[ИдентификаторУдаляемогоОбработчика].ИмяОбработчика
			);
			Протокол.ГенераторПротокола.ДобавитьСтрокуВПротокол(Протокол, СтрокаПротокола);
			
			ЭлементФормы.ОбработчикиЭлемента.Удалить(ИдентификаторУдаляемогоОбработчика);
		КонецЦикла;
	КонецЦикла;
КонецПроцедуры
...
Процедура МодифицироватьРеквизиты(Форма, ОписаниеМодификаций, Протокол)
	Для Каждого Элемент Из ОписаниеМодификаций.Реквизиты Цикл
		...
		НовыйРеквизит = Новый РеквизитФормы(ОписаниеРеквизита.ИмяРеквизита, ТипыРеквизита, ОписаниеРеквизита.Родитель);
		...
		Исключение
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			Протокол.ГенераторПротокола.ДобавитьСтрокуВПротокол(Протокол, ИнформацияОбОшибке.Причина.Описание);
			Если Протокол.ПрерватьМодификациюПриОшибке Тогда
				ВызватьИсключение;
			КонецЕсли;
		КонецПопытки;
...
Процедура МодифицироватьКоманды(Форма, ОписаниеМодификаций, Протокол)
	Для Каждого Элемент Из ОписаниеМодификаций.Команды Цикл
		...
		Попытка
			...
			Для Каждого ЭлементКоллекции Из ОписаниеКоманды.СвойстваОбъекта Цикл
				КомандаФормы[ЭлементКоллекции.Ключ] = ЭлементКоллекции.Значение;
			КонецЦикла;
		Исключение
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			Протокол.ГенераторПротокола.ДобавитьСтрокуВПротокол(Протокол, ИнформацияОбОшибке.Описание);
...
Процедура МодифицироватьЭлементы(Форма, ОписаниеМодификаций, Протокол)
	...
	Для Каждого ЭлементКоллекции Из КоллекцияЭлементов Цикл
		...
		Для Каждого СвойствоЭлемента Из ОписаниеЭлемента.СвойстваОбъекта Цикл
			Попытка
		...
		Для Каждого Элемент Из ОписаниеЭлемента.ОбработчикиЭлемента Цикл
			...
			Исключение
				// BSLLS:NestedFunctionInParameters-off
				СтрокаПротокола = СтрШаблон(
					Нстр("ru='Ошибка установки обработчика ""%1"" события ""%2"" элемента ""%3""'"),
					ОписаниеОбработчика.ИмяПроцедурыОбработчика,
					ОписаниеОбработчика.ИмяОбработчика,
					ОписаниеЭлемента.ИмяЭлемента
				);
