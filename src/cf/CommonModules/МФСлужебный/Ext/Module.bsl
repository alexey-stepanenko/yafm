...
// РЕЖИМ РАЗРАБОТКИ: &После("ПередМодификациейФормы")
// РЕЖИМ РАЗРАБОТКИ: Процедура Разработка_ПередМодификациейФормы(Форма, ОписаниеМодификаций)
Процедура ПередМодификациейФормы(Форма, ОписаниеМодификаций) Экспорт
// РЕЖИМ РАЗРАБОТКИ: НАЧАЛО БЛОКА
	Если Форма.Параметры.Свойство(МФИнструментыРазработчикаКлиентСервер.ИмяПараметраСозданиеМодификации()) Тогда
		МФИнструментыРазработчика.ОбработатьДанныеФормыДляСозданияМодификации(Форма, ОписаниеМодификаций);
		ОписаниеМодификаций = НовыйОписаниеМодификаций();
	КонецЕсли;
// РЕЖИМ РАЗРАБОТКИ: КОНЕЦ БЛОКА
// РЕЖИМ РАЗРАБОТКИ: КонецПроцедуры
КонецПроцедуры

// РЕЖИМ РАЗРАБОТКИ: &После("ПослеМодификацииФормы")
// РЕЖИМ РАЗРАБОТКИ: Процедура Разработка_ПослеМодификацииФормы(Форма, ОписаниеМодификаций)
// РЕЖИМ ТЕСТИРОВАНИЯ: &После("ПослеМодификацииФормы")
// РЕЖИМ ТЕСТИРОВАНИЯ: Процедура Разработка_ПослеМодификацииФормы(Форма, ОписаниеМодификаций)
Процедура ПослеМодификацииФормы(Форма, ОписаниеМодификаций) Экспорт
// РЕЖИМ РАЗРАБОТКИ: НАЧАЛО БЛОКА
	Если Форма.Параметры.Свойство(МФИнструментыРазработчикаКлиентСервер.ИмяПараметраГенерацияАвтотестов()) Тогда
		МФИнструментыРазработчика.ОбработатьДанныеФормыДляГенерацииАвтотестов(Форма, ОписаниеМодификаций);
	КонецЕсли;
// РЕЖИМ РАЗРАБОТКИ: КОНЕЦ БЛОКА
// РЕЖИМ РАЗРАБОТКИ: КонецПроцедуры
// РЕЖИМ ТЕСТИРОВАНИЯ: НАЧАЛО БЛОКА
	Если Форма.Параметры.Свойство(МФТестированиеКлиентСерверПовтИсп.ИмяПараметраТестирования()) Тогда
		МФТестирование.ОбработатьДанныеФормыДляТестирования(Форма, ОписаниеМодификаций);
	КонецЕсли;
...
Функция НовыйОписаниеМодификаций()
	...
	Результат.Вставить("СвязиПараметровВыбора", Новый Структура);
