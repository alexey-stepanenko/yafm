...
Функция ВерсияИнструментовРазработчика() Экспорт
	Возврат "1.1.1.8"; // ВЕРСИЯ
КонецФункции

Функция ВозможенЗапуск(Сообщения) Экспорт
	Результат = Истина;
	
	ЭтоБезопасныйРежим = Ложь;
	БезопасныйРежим = БезопасныйРежим();
	Если ТипЗнч(БезопасныйРежим) = Тип("Строка") Тогда
		ЭтоБезопасныйРежим = Истина;
	Иначе
		ЭтоБезопасныйРежим = БезопасныйРежим;
	КонецЕсли;
	
	Если ЭтоБезопасныйРежим Тогда
		Сообщения.Добавить(НСтр("ru='Включен безопасный режим работы'"));
		Результат = Ложь;
	КонецЕсли;
	
	ВерсииРазличаются = (ВерсияИнструментовРазработчика() <> МФСлужебный.Версия());
	Если ВерсииРазличаются Тогда
		Сообщения.Добавить(СтрШаблон(
			НСтр("ru='Версия инструментов разработчика (%1) отличается от версии подсистемы (%2)'"),
			ВерсияИнструментовРазработчика(),
			МФСлужебный.Версия()
		));
		Результат = Ложь;
	КонецЕсли;
...
Процедура ОбработатьДанныеФормыДляСозданияМодификации(Форма, ОписаниеМодификаций) Экспорт
	АдресДанныхДляСозданияМодификации = Форма.Параметры[
		МФИнструментыРазработчикаКлиентСервер.ИмяПараметраАдресДанныхДляСозданияМодификации()
	];
	
	ОписаниеФормы = ОписаниеЭлементаФормыДляСозданияМодификации(Форма, "Форма");
	
	ДанныеДляСозданияМодификации = Новый Структура;
	ДанныеДляСозданияМодификации.Вставить("ОписаниеМодификаций", КопияСтруктуры(ОписаниеМодификаций));
	ДанныеДляСозданияМодификации.Вставить("ОписаниеФормы", ОписаниеФормы);
	
	ПоместитьВоВременноеХранилище(ДанныеДляСозданияМодификации, АдресДанныхДляСозданияМодификации);
КонецПроцедуры

Процедура ОбработатьДанныеФормыДляГенерацииАвтотестов(Форма, ОписаниеМодификаций) Экспорт
...
#Область КопированиеКоллекций

Функция КопияСтруктуры(ИсходнаяСтруктура)
	Результат = Новый Структура;
	
	Для Каждого ЭлементКоллекции Из ИсходнаяСтруктура Цикл
		Ключ = ЭлементКоллекции.Ключ;
		Значение = ЭлементКоллекции.Значение;
		ТипЗначения = ТипЗнч(Значение);
		
		Если ТипЗначения = Тип("Структура") Тогда
			ВременнаяСтруктура = КопияСтруктуры(Значение);
			Результат.Вставить(Ключ, ВременнаяСтруктура);
		ИначеЕсли ТипЗначения = Тип("Соответствие") Тогда
			ВременноеСоответствие = КопияСоответствия(Значение);
			Результат.Вставить(Ключ, ВременноеСоответствие);
		ИначеЕсли ТипЗначения = Тип("Массив") Тогда
			ВременныйМассив = КопияМассива(Значение);
			Результат.Вставить(Ключ, ВременныйМассив);
		Иначе
...
Функция КопияСоответствия(ИсходноеСоответствие)
	Результат = Новый Соответствие;
	
	Для Каждого ЭлементКоллекции Из ИсходноеСоответствие Цикл
		Ключ = ЭлементКоллекции.Ключ;
		Значение = ЭлементКоллекции.Значение;
		ТипЗначения = ТипЗнч(Значение);
...
Функция КопияМассива(ИсходныйМассив)
	...
	Для Каждого ЭлементКоллекции Из ИсходныйМассив Цикл
		...
		ИначеЕсли ТипЗначения = Тип("Соответствие") Тогда
			ВременноеСоответствие = КопияСоответствия(Значение);
			Результат.Добавить(ВременноеСоответствие);
		ИначеЕсли ТипЗначения = Тип("Массив") Тогда
			ВременныйМассив = КопияМассива(Значение);
...
#Область ОбработатьДанныеФормыДляСозданияМодификации

Функция ОписаниеЭлементаФормыДляСозданияМодификации(Элемент, Имя)
	Результат = Новый Структура;
	Результат.Вставить("Имя", Имя);
	Результат.Вставить("КоманднаяПанель", Неопределено);
	Результат.Вставить("ПодчиненныеЭлементы", Новый Массив);
	Результат.Вставить("ТипЭлемента", ТипЗнч(Элемент));
	
	Попытка
		КоманднаяПанель = Новый Массив;
		Для Каждого ЭлементКоманднойПанели Из Элемент.КоманднаяПанель.ПодчиненныеЭлементы Цикл
			// TODO 2026-04-13: Добавить обработку
		КонецЦикла;
		Результат.КоманднаяПанель = КоманднаяПанель;
	Исключение
	...
	Попытка
		Для Каждого ПодчиненныйЭлемент Из Элемент.ПодчиненныеЭлементы Цикл
			Результат.ПодчиненныеЭлементы.Добавить(
				ОписаниеЭлементаФормыДляСозданияМодификации(ПодчиненныйЭлемент, ПодчиненныйЭлемент.Имя)
			);
		КонецЦикла;
	Исключение
		// Обработка не требуется
