...
Процедура ПрочитатьНастройкиРаботыСРасширением()
	Для Каждого ИмяПараметра Из ИменаПараметров Цикл
		ЗначениеПараметра = ВосстановитьЗначение(СтрШаблон(
			"%1%2",
...
&НаСервере
Процедура СохранитьНастройкиРаботыСРасширением()
	Для Каждого ИмяПараметра Из ИменаПараметров Цикл
		...
		//СохранитьЗначение(ИмяСохраняемогоПараметра, ЭтаФорма[ИмяПараметра]);
	КонецЦикла;
КонецПроцедуры
