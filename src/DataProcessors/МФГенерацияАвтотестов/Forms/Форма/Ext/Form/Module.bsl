...
Асинх Процедура ЗагрузитьИзРасширения(Команда)
	...
	Для Каждого ПараметрИнициализации Из ПараметрыИнициализации Цикл
		Если ИменаПараметров.Найти(ПараметрИнициализации.Ключ) = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		ПараметрыИнициализации[ПараметрИнициализации.Ключ] = Объект[ПараметрИнициализации.Ключ];
	КонецЦикла;
...
Асинх Процедура ПослеУстановкиСвойствГенерацииРасширения(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	Ждать ГенераторРасширения.СоздатьРасширениеСТестами(ПолныйПутьКФайлуРасширения);
