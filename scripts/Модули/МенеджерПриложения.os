...
Функция ВыполнитьКоманду(Знач ИмяКоманды, Знач ПараметрыКоманды) Экспорт
	...
	ОписаниеКоманды = ПарсерКоманд.ПолучитьКоманду(ИмяКоманды);
	
	ПараметрыВыполненияКоманды = Новый Структура;
	Для Каждого ПараметрКоманды Из НастройкаПриложения.ПараметрыКоманд() Цикл
		Если
			ПараметрКомандыВТаблицеПараметров(ПараметрКоманды, ОписаниеКоманды.ИменованныеПараметры)
			Или ПараметрКомандыВТаблицеПараметров(ПараметрКоманды, ОписаниеКоманды.ПозиционныеПараметры)
		Тогда
			ПараметрыВыполненияКоманды.Вставить(ПараметрКоманды.Ключ, Неопределено);
		КонецЕсли;
	КонецЦикла;
	ПутьКФайлуНастроек = ПараметрыКоманды.Получить(НастройкаПриложения.ПараметрыКоманд().ФайлНастроек.ИмяВКоманднойСтроке);
	Если ПутьКФайлуНастроек <> Неопределено Тогда
		Если ФС.ФайлСуществует(ПутьКФайлуНастроек) Тогда
			ТекстовыйДокумент = Новый ТекстовыйДокумент;
			ТекстовыйДокумент.Прочитать(ПутьКФайлуНастроек);
			ЧтениеJSON = Новый ЧтениеJSON;
			ЧтениеJSON.УстановитьСтроку(ТекстовыйДокумент.ПолучитьТекст());
			ПараметрыИзФайла = ПрочитатьJSON(ЧтениеJSON, Истина);
			Для Каждого ПараметрКоманды Из ПараметрыВыполненияКоманды Цикл
				ЗначениеПараметра = ПараметрыИзФайла.Получить(
					НастройкаПриложения.ПараметрыКоманд()[ПараметрКоманды.Ключ].ИмяВФайлеНастроек
				);
				Если ЗначениеПараметра = Неопределено Тогда
					Продолжить;
				КонецЕсли;
				ПараметрыВыполненияКоманды[ПараметрКоманды.Ключ] = ЗначениеПараметра;
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
	Для Каждого ПараметрКоманды Из ПараметрыВыполненияКоманды Цикл
		ЗначениеПараметра = ПараметрыКоманды.Получить(
			НастройкаПриложения.ПараметрыКоманд()[ПараметрКоманды.Ключ].ИмяВКоманднойСтроке
		);
		Если ЗначениеПараметра = Неопределено Тогда
			Продолжить;
		КонецЕсли;
	...
	КодВозврата = Команда.ВыполнитьКоманду(ПараметрыВыполненияКоманды, ЭтотОбъект);
...
Функция ПараметрКомандыВТаблицеПараметров(ПараметрКоманды, ТаблицаПараметров)
	НайденныеСтроки = ТаблицаПараметров.НайтиСтроки(Новый Структура("Имя", ПараметрКоманды.Значение.ИмяВКоманднойСтроке));
