...
Процедура Инициализировать(ПараметрыКонструктора) Экспорт
    ...
    ПараметрыКонструктораРасширения.ИдентификаторБлокаКодаРасширения = НастройкаПриложения.БлокРазработки;
...
Функция БлокиКУдалению() Экспорт
    ...
    Результат.Добавить(НастройкаПриложения.БлокТестирования);
...
Процедура ВыполнитьДополнительныеДействия() Экспорт
	...
	ПутьКФайлуКомандногоИнтерфейса = ОбъединитьПути(
        КаталогИсходников,
        "Ext", // BSLLS:DuplicateStringLiteral-off
    ...
    СоставПутиКФайлуКомандногоИнтерфейсаОсновнойПодсистемы.Добавить("Ext"); // BSLLS:DuplicateStringLiteral-off
    ...
    ЗначениеВидимости.Вставить("xr:Common", Ложь);
