...
Процедура ОбработатьПараметрыСозданияРелизов(ПараметрыСозданияРелизов)
	...
	Если ПустаяСтрока(ПараметрыСозданияРелизов.ВариантыПоставок) Тогда
		ПараметрыСозданияРелизов.ВариантыПоставок = НастройкаПриложения.ВариантыПоставокПоУмолчанию;
	КонецЕсли;
...
Процедура СформироватьВременныйКаталогИсходников(ВременныйКаталогИсходников, КаталогИсходников)
	...
	УдалитьФайлы(ОбъединитьПути(ВременныйКаталогИсходников, НастройкаПриложения.ИмяФайлаДампКонфигурации));

	НайденныеФайлы = НайтиФайлы(ВременныйКаталогИсходников, "*.bsl", Истина);
	РегулярноеВыражение = РегЭкспПоискСлужебныхКомментариев(НастройкаПриложения.ПрефиксыУдаляемыхКомментариев);
	Для Каждого НайденныйФайл Из НайденныеФайлы Цикл
		УдалитьСлужебныеКомментарииИзТекстовМодулей(НайденныйФайл.ПолноеИмя, РегулярноеВыражение);
	КонецЦикла;

	ПутьКФайлуКонфигурации = ОбъединитьПути(ВременныйКаталогИсходников, НастройкаПриложения.ИмяФайлаКонфигурации);
...
Функция ТекстРегулярногоВыраженияУдаляемогоБлока(УдаляемыйБлок)
	Возврат СтрШаблон(НастройкаПриложения.ШаблонРегулярногоВыраженияПоискаБлока, УдаляемыйБлок);
КонецФункции
...
Функция НомерРелиза(_КаталогИсходников)
	Результат = "";
    ПроцессорXML = Новый СериализацияДанныхXML(Ложь);
	ПутьКФайлуКонфигурации = ОбъединитьПути(_КаталогИсходников, НастройкаПриложения.ИмяФайлаКонфигурации);
