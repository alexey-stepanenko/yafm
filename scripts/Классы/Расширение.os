...
Процедура СкорректироватьФайлКонфигурации(ОбъектыПоставки) Экспорт
    ...
    УдалитьСвойстваКонфигурации(Конфигурация);
...
Процедура СкорректироватьТекстМодуля(ПутьКФайлуМодуля, СодержимоеФайла) Экспорт
	...
	ТекстРегулярногоВыражения = СтрШаблон(
        НастройкаПриложения.ШаблонРегулярногоВыраженияПоискаБлока,
    ...
    Для Каждого СтрокаФайла Из СоставИсходногоФайла Цикл
        ...
        ИначеЕсли Совпадения.Количество() > 0 Тогда
            ...
            Если ЗначениеВСтроке = НастройкаПриложения.НачалоБлока Тогда
                НашлиБлок = Истина;
            ИначеЕсли ЗначениеВСтроке = НастройкаПриложения.КонецБлока Тогда
                НашлиБлок = Ложь;
            Иначе
...
Процедура УдалитьСвойстваКонфигурации(Конфигурация)
    Коллекция = Конфигурация
        .Получить("MetaDataObject")
        ._Элементы
        .Получить("Configuration")
    ...
    УдаляемыеСвойства.Добавить("DefaultLanguage"); // Язык по умолчанию
    УдаляемыеСвойства.Добавить("ModalityUseMode"); // Режим использования модальности
    // Режим использования синхронных вызовов расширений платформы и внешних компонент
    УдаляемыеСвойства.Добавить("SynchronousPlatformExtensionAndAddInCallUseMode");
    УдаляемыеСвойства.Добавить("InterfaceCompatibilityMode"); // Режим совместимости интерфейса
    УдаляемыеСвойства.Добавить("DatabaseTablespacesUseMode"); // Режим использования табличных пространств
    УдаляемыеСвойства.Добавить("CompatibilityMode"); // Режим совместимости
    
    Для Каждого УдаляемоеСвойство Из УдаляемыеСвойства Цикл
        Коллекция.Удалить(УдаляемоеСвойство);
    КонецЦикла;
КонецПроцедуры
...
Процедура СкорректироватьОбъектыКонфигурации(Конфигурация, ОбъектыПоставки)
    ...
    УдаляемыеОбъекты.Добавить("Language.");
    Для Каждого ИмяОбъекта Из ОбъектыПоИмени Цикл
        НашлиУдаляемыйОбъект = Ложь;
        Для Каждого УдаляемыйОбъект Из УдаляемыеОбъекты Цикл
            Если Лев(ИмяОбъекта, СтрДлина(УдаляемыйОбъект)) = УдаляемыйОбъект Тогда
                НашлиУдаляемыйОбъект = Истина;
                Прервать;
            КонецЕсли;
        КонецЦикла;
        Если НашлиУдаляемыйОбъект Тогда
            Продолжить;
        КонецЕсли;
