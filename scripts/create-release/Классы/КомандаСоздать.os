...
Процедура НастроитьКоманду(Знач Команда, Знач Парсер) Экспорт
	...
	Парсер.ДобавитьИменованныйПараметрКоманды(Команда, ПараметрВариантыПоставок(), "Варианты поставок");
	Парсер.ДобавитьПараметрФлагКоманды(Команда, ПараметрСоздатьКаталогНомераРелиза(), "Создавать отдельный каталог с номером релиза");
...
Функция ВыполнитьКоманду(Знач ПараметрыКоманды, Знач Приложение) Экспорт
	...
	ВариантыПоставок = Строка(ПараметрыКоманды.Получить(ПараметрВариантыПоставок()));
	СоздатьКаталогНомераРелиза = ПараметрыКоманды.Получить(ПараметрСоздатьКаталогНомераРелиза());
	...
	Поставки = СтрРазделить(ВариантыПоставок, ",");
	КонструкторыРелизов = Новый Массив();

	Если Поставки.Найти("Поставка") <> Неопределено Тогда
		КонструкторыРелизов.Добавить(Поставка);
	КонецЕсли;
	Если Поставки.Найти("РасширениеРазработчикаДляПоставки") <> Неопределено Тогда
		КонструкторыРелизов.Добавить(РасширениеРазработчикаДляПоставки);
	КонецЕсли;

	Если КонструкторыРелизов.Количество() = 0 Тогда
		КонструкторыРелизов.Добавить(Поставка);
		КонструкторыРелизов.Добавить(РасширениеРазработчикаДляПоставки);
	КонецЕсли;
	...
	ПараметрыКонструктора.ПрефиксИмениФайлаРелиза = ПрефиксИмениФайлаРелиза;
	ПараметрыКонструктора.ИмяФайлаКонфигурации = ИмяФайлаКонфигурации();
...
Функция ПараметрВариантыПоставок()
	Возврат "-distrib-variants";
КонецФункции

Функция ПараметрСоздатьКаталогНомераРелиза()
	Возврат "-create-number-release-dir";
КонецФункции
...
Функция ПрефиксыУдаляемыхКомментариев()
	Результат = Новый Массив;

	Результат.Добавить("TODO");
...
Функция НовыйПараметрыКонструктора()
	...
	Результат.Вставить("ПрефиксИмениФайлаРелиза", Неопределено);
	Результат.Вставить("ИмяФайлаКонфигурации", Неопределено);
...
Процедура СформироватьВременныйКаталогИсходников(ВременныйКаталогИсходников, КаталогИсходников)
	...
	Маркеры = ПрефиксыУдаляемыхКомментариев();
	...
	ПутьКФайлуКонфигурации = ОбъединитьПути(
		ВременныйКаталогИсходников,
...
Процедура СкорректироватьФайлКонфигурации(ПолноеИмяФайла)
	...
	Для Каждого ОчищаемаяВетка Из ОчищаемыеВетки Цикл
		...
		Ветка = НайтиОчищаемуюВетку(КонфигурацияПодсистемы, СтрСоединить(СоставПути, ","));
		Ветка.Вставить(ИмяВетки, Неопределено);
	КонецЦикла;
	ПроцессорXML.ЗаписатьВФайл(КонфигурацияПодсистемы, ПолноеИмяФайла);
КонецПроцедуры

Функция НайтиОчищаемуюВетку(Ветки, ПутьКОчищаемойВетке)
	СоставПути = СтрРазделить(ПутьКОчищаемойВетке, ",");
	...
	Если ТипЗнч(Ветки) = Тип("Соответствие") Тогда
		Если Ветки.Получить(ИмяВетки) <> Неопределено Тогда
			...
			Иначе
				Возврат НайтиОчищаемуюВетку(Ветки.Получить(ИмяВетки), ПутьКНижестоящейВетке(ПутьКОчищаемойВетке));
			КонецЕсли;
		Иначе
	...
	ИначеЕсли ТипЗнч(Ветки) = Тип("Структура") Тогда
		Если Ветки.Свойство("_Элементы") Тогда
			Возврат НайтиОчищаемуюВетку(Ветки._Элементы, ПутьКОчищаемойВетке);
		Иначе
...
Функция ТекстИсправленногоФайла(СтрокиФайла, РегулярноеВыражение)
	...
	Для Каждого СтрокаФайла Из СтрокиФайла Цикл
		...
		Если Совпадения.Количество() = 1 Тогда
			...
			Если ЗначениеВСтроке = Идентификаторы.НачалоБлока() Тогда
				НашлиУдаляемыйБлок = Истина;
			ИначеЕсли ЗначениеВСтроке = Идентификаторы.КонецБлока() Тогда
				НашлиУдаляемыйБлок = Ложь;
			Иначе // BSLLS:EmptyCodeBlock-off
				// Пустой блок
			КонецЕсли;
...
Функция ТекстРегулярногоВыраженияУдаляемогоБлока(УдаляемыйБлок)
	Возврат СтрШаблон(Идентификаторы.ШаблонРегулярногоВыраженияПоискаБлока(), УдаляемыйБлок);
КонецФункции
